...
async def batch_update_ks_video_comments(video_id: str, comments: List[Dict]):
    config.logger.info(f"[store.kuaishou.batch_update_ks_video_comments] video_id:{video_id}, comments:{comments}")
    if not comments:
        return
...
async def save_creator(user_id: str, creator: Dict):
    ...
    config.logger.info(f"[store.kuaishou.save_creator] creator:{local_db_item}")
