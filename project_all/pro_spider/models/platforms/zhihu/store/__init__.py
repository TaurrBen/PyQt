...
async def update_zhihu_content(content_item: ZhihuContent):
    ...
    local_db_item.update({"last_modify_ts": time_utils.get_current_timestamp()})
    config.logger.info(f"[store.zhihu.update_zhihu_content] zhihu content: {local_db_item}")
...
async def update_zhihu_content_comment(comment_item: ZhihuComment):
    ...
    local_db_item.update({"last_modify_ts": time_utils.get_current_timestamp()})
    config.logger.info(f"[store.zhihu.update_zhihu_note_comment] zhihu content comment:{local_db_item}")
