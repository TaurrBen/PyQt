...
class ZhiHuClient(AbstractApiClient):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        if response.status_code != 200:
            config.logger.error(f"[ZhiHuClient.request] Requset Url: {url}, Request error: {response.text}")
        ...
        try:
            ...
            if data.get("error"):
                config.logger.error(f"[ZhiHuClient.request] Request error: {data}")
                raise DataFetchError(data.get("error", {}).get("message"))
            return data
        except json.JSONDecodeError:
            config.logger.error(f"[ZhiHuClient.request] Request error: {response.text}")
            raise DataFetchError(response.text)
    ...
    async def pong(self) -> bool:
        ...
        """
        config.logger.info("[ZhiHuClient.pong] Begin to pong zhihu...")
        ping_flag = False
        try:
            res = await self.get_current_user_info()
            if res.get("uid") and res.get("name"):
                ping_flag = True
                config.logger.info("[ZhiHuClient.pong] Ping zhihu successfully")
            else:
                config.logger.error(f"[ZhiHuClient.pong] Ping zhihu failed, response data: {res}")
        except Exception as e:
            config.logger.error(f"[ZhiHuClient.pong] Ping zhihu failed: {e}, and try to login again...")
            ping_flag = False
        return ping_flag
    ...
    async def get_note_by_keyword(
        ...
        config.logger.info(f"[ZhiHuClient.get_note_by_keyword] Search result: {search_res}")
    ...
    async def get_all_anwser_by_creator(self, creator: ZhihuCreator, crawl_interval: float = 1.0,
        ...
        while not is_end:
            ...
            if not res:
                break
            config.logger.info(f"[ZhiHuClient.get_all_anwser_by_creator] Get creator {creator.url_token} answers: {res}")
