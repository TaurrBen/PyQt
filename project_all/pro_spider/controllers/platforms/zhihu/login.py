...
class ZhiHuLogin(AbstractLogin):
    ...
    async def begin(self,login_type:str):
        """Start login zhihu"""
        config.logger.info("[ZhiHu.begin] Begin login zhihu ...")
    ...
    async def login_by_qrcode(self):
        ...
        config.logger.info("[ZhiHu.login_by_qrcode] Begin login zhihu by qrcode ...")
        qrcode_img_selector = "canvas.Qrcode-qrcode"
        # find login qrcode
        base64_qrcode_img = await find_qrcode_img_from_canvas(
            self.context_page,
            canvas_selector=qrcode_img_selector
        )
        if not base64_qrcode_img:
            config.logger.info("[ZhiHu.login_by_qrcode] login failed , have not found qrcode please check ....")
            if not base64_qrcode_img:
                sys.exit()
        ...
        config.logger.info(f"[ZhiHu.login_by_qrcode] waiting for scan code login, remaining time is 120s")
        try:
            await self.check_login_state()

        except RetryError:
            config.logger.info("[ZhiHu.login_by_qrcode] Login zhihu failed by qrcode login method ...")
            sys.exit()
    ...
    async def login_by_cookies(self):
        ...
        config.logger.info("[ZhiHu.login_by_cookies] Begin login zhihu by cookie ...")
