...
class ZhihuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        if True:
            ...
            config.logger.info("[ZhihuCrawler.start] Zhihu跳转到搜索页面获取搜索页面的Cookies，该过程需要5秒左右")
    ...
    async def batch_get_content_comments(self, content_list: List[ZhihuContent]):
        ...
        if not config.ENABLE_GET_COMMENTS:
            config.logger.info(f"[ZhihuCrawler.batch_get_content_comments] Crawling comment mode is not enabled")
            return
    ...
    async def get_comments(self, content_item: ZhihuContent, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            config.logger.info(f"[ZhihuCrawler.get_comments] Begin get note id comments {content_item.content_id}")
    ...
    async def get_creators_and_notes(self) -> None:
        ...
        """
        config.logger.info("[ZhihuCrawler.get_creators_and_notes] Begin get xiaohongshu creators")
        for user_link in config.ZHIHU_CREATOR_URL_LIST:
            config.logger.info(f"[ZhihuCrawler.get_creators_and_notes] Begin get creator {user_link}")
            ...
            if not createor_info:
                config.logger.info(f"[ZhihuCrawler.get_creators_and_notes] Creator {user_url_token} not found")
                continue

            config.logger.info(f"[ZhihuCrawler.get_creators_and_notes] Creator info: {createor_info}")
    ...
    async def create_zhihu_client(self, httpx_proxy: Optional[str]) -> ZhiHuClient:
        """Create zhihu client"""
        config.logger.info("[ZhihuCrawler.create_zhihu_client] Begin create zhihu API client ...")
    ...
    async def launch_browser(
        ...
        config.logger.info("[ZhihuCrawler.launch_browser] Begin create browser context ...")
    ...
    async def close(self):
        ...
        await self.browser_context.close()
        config.logger.info("[ZhihuCrawler.close] Browser context closed ...")
