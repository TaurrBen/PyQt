...
class DouyinLogin(AbstractLogin):
    ...
    async def begin(self,login_type:str):
        ...
        # check login state
        config.logger.info(f"[DouYinLogin.begin] login finished then check login state ...")
        try:
            await self.check_login_state()
        except RetryError:
            config.logger.info("[DouYinLogin.begin] login failed please confirm ...")
            sys.exit()

        # wait for redirect
        wait_redirect_seconds = 5
        config.logger.info(f"[DouYinLogin.begin] Login successful then wait for {wait_redirect_seconds} seconds redirect ...")
    ...
    async def popup_login_dialog(self):
        ...
        try:
            ...
            config.logger.error(f"[DouYinLogin.popup_login_dialog] login dialog box does not pop up automatically, error: {e}")
            config.logger.info("[DouYinLogin.popup_login_dialog] login dialog box does not pop up automatically, we will manually click the login button")
            login_button_ele = self.context_page.locator("xpath=//p[text() = '登录']")
            await login_button_ele.click()
            await asyncio.sleep(0.5)

    async def login_by_qrcode(self):
        config.logger.info("[DouYinLogin.login_by_qrcode] Begin login douyin by qrcode...")
        ...
        if not base64_qrcode_img:
            config.logger.info("[DouYinLogin.login_by_qrcode] login qrcode not found please confirm ...")
            sys.exit()
    ...
    async def login_by_mobile(self):
        config.logger.info("[DouYinLogin.login_by_mobile] Begin login douyin by mobile ...")
        ...
        while max_get_sms_code_time > 0:
            config.logger.info(f"[DouYinLogin.login_by_mobile] get douyin sms code from redis remaining time {max_get_sms_code_time}s ...")
    ...
    async def check_page_display_slider(self, move_step: int = 10, slider_level: str = "easy"):
        ...
        while not slider_verify_success:
            if max_slider_try_times <= 0:
                config.logger.error("[DouYinLogin.check_page_display_slider] slider verify failed ...")
                sys.exit()
            try:
                ...
                if "操作过慢" in page_content or "提示重新操作" in page_content:
                    config.logger.info("[DouYinLogin.check_page_display_slider] slider verify failed, retry ...")
                ...
                config.logger.info("[DouYinLogin.check_page_display_slider] slider verify success ...")
                slider_verify_success = True
            except Exception as e:
                config.logger.error(f"[DouYinLogin.check_page_display_slider] slider verify failed, error: {e}")
                await asyncio.sleep(1)
                max_slider_try_times -= 1
                config.logger.info(f"[DouYinLogin.check_page_display_slider] remaining slider try times: {max_slider_try_times}")
                continue
    ...
    async def login_by_cookies(self):
        config.logger.info("[DouYinLogin.login_by_cookies] Begin login douyin by cookie ...")
