...
class WeiboCrawler(AbstractCrawler):
    ...
    async def get_note_info_task(self, note_id: str, semaphore: asyncio.Semaphore) -> Optional[Dict]:
        ...
        async with semaphore:
            try:
                ...
                config.logger.error(f"[WeiboCrawler.get_note_info_task] Get note detail error: {ex}")
                return None
    ...
    async def batch_get_notes_comments(self, note_id_list: List[str]):
        ...
        if not config.ENABLE_GET_COMMENTS:
            config.logger.info(f"[WeiboCrawler.batch_get_note_comments] Crawling comment mode is not enabled")
            return

        config.logger.info(f"[WeiboCrawler.batch_get_notes_comments] note ids:{note_id_list}")
        semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
        task_list: List[Task] = []
    ...
    async def get_note_comments(self, note_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            try:
                config.logger.info(f"[WeiboCrawler.get_note_comments] begin get note_id: {note_id} comments ...")
                ...
                config.logger.error(f"[WeiboCrawler.get_note_comments] get note_id: {note_id} comment error: {ex}")
            except Exception as e:
                config.logger.error(f"[WeiboCrawler.get_note_comments] may be been blocked, err:{e}")
    ...
    async def get_note_images(self, mblog: Dict):
        ...
        if not config.ENABLE_GET_IMAGES:
            config.logger.info(f"[WeiboCrawler.get_note_images] Crawling image mode is not enabled")
            return
    ...
    async def get_creators_and_notes(self) -> None:
        ...
        for user_id in config.WEIBO_CREATOR_ID_LIST:
            ...
            if createor_info_res:
                ...
                config.logger.info(f"[WeiboCrawler.get_creators_and_notes] creator info: {createor_info}")
    ...
    async def create_weibo_client(self, httpx_proxy: Optional[str]) -> WeiboClient:
        """Create xhs client"""
        config.logger.info("[WeiboCrawler.create_weibo_client] Begin create weibo API client ...")
