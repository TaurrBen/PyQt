...
# version    ：python 3.9
# Description：微博爬虫 API 请求 client
"""
...
class WeiboClient:
    ...
    async def request(self, method, url, **kwargs) -> Union[Response, Dict]:
        ...
        if ok_code == 0:  # response error
            config.logger.error(f"[WeiboClient.request] request {method}:{url} err, res:{data}")
    ...
    async def pong(self) -> bool:
        ...
        config.logger.info("[WeiboClient.pong] Begin pong weibo...")
        ping_flag = False
        try:
            uri = "/api/config"
            ...
            if resp_data.get("login"):
                ...
                config.logger.error(f"[WeiboClient.pong] cookie may be invalid and again login...")
        except Exception as e:
            config.logger.error(f"[WeiboClient.pong] Pong weibo failed: {e}, and try to login again...")
            ping_flag = False
        return ping_flag
    ...
    async def get_note_info_by_id(self, note_id: str) -> Dict:
        ...
        async with httpx.AsyncClient(proxies=self.proxies) as client:
            ...
            if match:
                ...
                config.logger.info(f"[WeiboClient.get_note_info_by_id] 未找到$render_data的值")
                return dict()
    ...
    async def get_note_image(self, image_url: str) -> bytes:
        ...
        async with httpx.AsyncClient(proxies=self.proxies) as client:
            ...
            if not response.reason_phrase == "OK":
                config.logger.error(f"[WeiboClient.get_note_image] request {final_uri} err, res:{response.text}")
                return None
            else:
    ...
    async def get_creator_info_by_id(self, creator_id: str) -> Dict:
        ...
        if container_info.get("fid_container_id") == "" or container_info.get("lfid_container_id") == "":
            config.logger.error(f"[WeiboClient.get_creator_info_by_id] get containerid failed")
            raise DataFetchError("get containerid failed")
        params = {
            "jumpfrom": "weibocom",
            "type": "uid",
