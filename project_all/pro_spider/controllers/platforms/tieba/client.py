...
class BaiduTieBaClient(AbstractApiClient):
    ...
    async def request(self, method, url, return_ori_content=False, proxies=None, **kwargs) -> Union[str, Any]:
        ...
        if response.status_code != 200:
            config.logger.error(f"Request failed, method: {method}, url: {url}, status code: {response.status_code}")
            config.logger.error(f"Request failed, response: {response.text}")
    ...
    async def get(self, uri: str, params=None, return_ori_content=False, **kwargs) -> Any:
        ...
        try:
            ...
            if self.ip_pool:
                ...
                _, proxies = format_proxy_info(proxie_model)
                res = await self.request(method="GET", url=f"{self._host}{final_uri}",
                                         return_ori_content=return_ori_content,
                                         proxies=proxies,
                                         **kwargs)
            ...
            config.logger.error(f"[BaiduTieBaClient.get] 达到了最大重试次数，IP已经被Block，请尝试更换新的IP代理: {e}")
    ...
    async def pong(self) -> bool:
        ...
        """
        config.logger.info("[BaiduTieBaClient.pong] Begin to pong tieba...")
        try:
            uri = "/mo/q/sync"
            res: Dict = await self.get(uri)
            config.logger.info(f"[BaiduTieBaClient.pong] res: {res}")
            if res and res.get("no") == 0:
                ping_flag = True
            else:
                config.logger.info(f"[BaiduTieBaClient.pong] user not login, will try to login again...")
                ping_flag = False
        except Exception as e:
            config.logger.error(f"[BaiduTieBaClient.pong] Ping tieba failed: {e}, and try to login again...")
            ping_flag = False
        return ping_flag
