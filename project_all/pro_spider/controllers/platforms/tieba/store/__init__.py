...
from .m_tieba import TiebaComment, TiebaCreator, TiebaNote
...
async def update_tieba_note(note_item: TiebaNote):
    ...
    save_note_item.update({"last_modify_ts": time_utils.get_current_timestamp()})
    config.logger.info(f"[store.tieba.update_tieba_note] tieba note: {save_note_item}")
...
async def update_tieba_note_comment(note_id: str, comment_item: TiebaComment):
    ...
    save_comment_item.update({"last_modify_ts": time_utils.get_current_timestamp()})
    config.logger.info(f"[store.tieba.update_tieba_note_comment] tieba note id: {note_id} comment:{save_comment_item}")
...
async def save_creator(user_info: TiebaCreator):
    ...
    local_db_item["last_modify_ts"] = time_utils.get_current_timestamp()
    config.logger.info(f"[store.tieba.save_creator] creator:{local_db_item}")
