...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def search_by_keyword(self):
        ...
        if self.params:
            ...
            for keyword in self.params.get("keyword"):
                ...
                config.logger.info(f"[BilibiliCrawler.search] Current search keyword: {keyword}")
                if not self.params.get("start_page")[0]:
                    pass
                for day in pd.date_range(start=self.params.get("pubtime_begin_s"), end=self.params.get("pubtime_end_s"), freq='D'):
                    ...
                    while (page - self.params.get("start_page")[1] + 1) * bili_limit_count <= self.params.get("video_count"):
                        ...
                        try:
                            ...
                            for video_item in video_items:
                                if video_item:
                                    ...
                                    save_content_item = await bilibili_store.update_bilibili_video(video_item,self.params.get("video_items_is_save"))
                                    event = ViewDataEvent("self", None, [keyword]+list(save_content_item.values()),
                                                          "self.tableWidget_video_items_addRow(event.data)")
                                    QCoreApplication.postEvent(self.parent.ui, event)

                                    saver_up_info = await bilibili_store.update_up_info(video_item,self.params.get("video_upuser_items_is_save"))
                        ...
                        # go to next day
                            ...
                            print(e)
                            break
        config.logger.info("[BilibiliCrawler.start] Bilibili Crawler finished ...")
        event = ViewDataEvent("pushButton_load_params", None, None,
                              "qwidget.setEnabled(True)")
        QCoreApplication.postEvent(self.parent.ui, event)
        event = ViewDataEvent("pushButton_start_search", None, None,
    ...
    async def search(self):
        ...
        """
        config.logger.info("[BilibiliCrawler.search] Begin search bilibli keywords")
        bili_limit_count = 36  # bilibili limit page fixed value
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            if not config.ALL_DAY:
                ...
                while (page - start_page + 1) * bili_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                    if page < start_page:
                        config.logger.info(f"[BilibiliCrawler.search] Skip page: {page}")
                        page += 1
                        continue

                    config.logger.info(f"[BilibiliCrawler.search] search bilibili keyword: {keyword}, page: {page}")
                    video_id_list: List[str] = []
                    ...
                    semaphore = asyncio.Semaphore(self.params.get("concurrency_num",1))
                    task_list = [self.get_video_info_task(aid=video_item.get("aid"), bvid="", semaphore=semaphore) for
                                 video_item in video_list]
                    video_items = await asyncio.gather(*task_list)
                    for video_item in video_items:
                        if video_item:
    ...
    async def get_bilibili_video(self, video_item: Dict, semaphore: asyncio.Semaphore):
        ...
        #     config.logger.info(f"[BilibiliCrawler.get_bilibili_video] Crawling image mode is not enabled")
        #     return
        ...
        if result is None:
            config.logger.info("[BilibiliCrawler.get_bilibili_video] get video play url failed")
            return
        ...
        if video_url == "":
            config.logger.info("[BilibiliCrawler.get_bilibili_video] get video url failed")
            return
