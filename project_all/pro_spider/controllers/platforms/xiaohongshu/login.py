...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def check_login_state(self, no_logged_in_session: str) -> bool:
        ...
        if "请通过验证" in await self.context_page.content():
            config.logger.info("[XiaoHongShuLogin.check_login_state] 登录过程中出现验证码，请手动验证")
    ...
    async def begin(self,login_type:str):
        ...
        config.logger.info("[XiaoHongShuLogin.begin] Begin login xiaohongshu ...")
    ...
    async def login_by_mobile(self):
        ...
        config.logger.info("[XiaoHongShuLogin.login_by_mobile] Begin login xiaohongshu by mobile ...")
        ...
        try:
            ...
            config.logger.info("[XiaoHongShuLogin.login_by_mobile] have not found mobile button icon and keep going ...")
        ...
        while max_get_sms_code_time > 0:
            config.logger.info(f"[XiaoHongShuLogin.login_by_mobile] get sms code from redis remaining time {max_get_sms_code_time}s ...")
            await asyncio.sleep(1)
        ...
        try:
            ...
            config.logger.info("[XiaoHongShuLogin.login_by_mobile] Login xiaohongshu failed by mobile login method ...")
            sys.exit()

        wait_redirect_seconds = 5
        config.logger.info(f"[XiaoHongShuLogin.login_by_mobile] Login successful then wait for {wait_redirect_seconds} seconds redirect ...")
        await asyncio.sleep(wait_redirect_seconds)

    async def login_by_qrcode(self):
        """login xiaohongshu website and keep webdriver login state"""
        config.logger.info("[XiaoHongShuLogin.login_by_qrcode] Begin login xiaohongshu by qrcode ...")
        ...
        if not base64_qrcode_img:
            config.logger.info("[XiaoHongShuLogin.login_by_qrcode] login failed , have not found qrcode please check ....")
        ...
        config.logger.info(f"[XiaoHongShuLogin.login_by_qrcode] waiting for scan code login, remaining time is 120s")
        try:
            await self.check_login_state(no_logged_in_session)
        except RetryError:
            config.logger.info("[XiaoHongShuLogin.login_by_qrcode] Login xiaohongshu failed by qrcode login method ...")
            sys.exit()

        wait_redirect_seconds = 5
        config.logger.info(f"[XiaoHongShuLogin.login_by_qrcode] Login successful then wait for {wait_redirect_seconds} seconds redirect ...")
        await asyncio.sleep(wait_redirect_seconds)

    async def login_by_cookies(self):
        """login xiaohongshu website by cookies"""
        config.logger.info("[XiaoHongShuLogin.login_by_cookies] Begin login xiaohongshu by cookie ...")
