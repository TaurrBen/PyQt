...
from .xiaohongshu_store_image import *
from .xiaohongshu_store_impl import *
...
async def update_xhs_note(note_item: Dict):
    ...
    local_db_item = {
        ...
        "last_modify_ts": time_utils.get_current_timestamp(), # 最后更新时间戳（MediaCrawler程序生成的，主要用途在db存储的时候记录一条记录最新更新时间）
    ...
    config.logger.info(f"[store.xhs.update_xhs_note] xhs note: {local_db_item}")
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    config.logger.info(f"[store.xhs.update_xhs_note_comment] xhs note comment:{local_db_item}")
...
async def save_creator(user_id: str, creator: Dict):
    ...
    config.logger.info(f"[store.xhs.save_creator] creator:{local_db_item}")
