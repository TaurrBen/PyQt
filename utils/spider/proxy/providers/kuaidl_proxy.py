...
"""
# File       : kuaidl_proxy.py
# Time       ：2025.2.22 11:08
# Author     ：Benboy
    ...
    async def get_proxies(self, num: int) -> List[IpInfoModel]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                config.logger.error(f"[KuaiDaiLiProxy.get_proxies] statuc code not 200 and response.txt:{response.text}")
            ...
            if ip_response.get("code") != 0:
                config.logger.error(f"[KuaiDaiLiProxy.get_proxies]  code not 0 and msg:{ip_response.get('msg')}")
