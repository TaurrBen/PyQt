...
"""
# File       : proxy_ip_pool.py
# Time       ：2025.2.22 10:51
# Author     ：Benboy
...
class ProxyIpPool():
    ...
    async def _is_valid_proxy(self, proxy: IpInfoModel) -> bool:
        ...
        :return:
        """
        config.logger.info(f"[ProxyIpPool._is_valid_proxy] testing {proxy.ip} is it valid ")
        try:
            httpx_proxy = {
            ...
            config.logger.info(f"[ProxyIpPool._is_valid_proxy] testing {proxy.ip} err: {e}")
            raise e
...
async def create_ip_pool(ip_pool_count: int, enable_validate_ip: bool,proxy_name: str) -> ProxyIpPool:
    """
     创建 IP 代理池
    ...
    pool = ProxyIpPool(ip_pool_count=ip_pool_count,
                       ...
                       ip_provider=IpProxyProvider.get(proxy_name)
                       )
