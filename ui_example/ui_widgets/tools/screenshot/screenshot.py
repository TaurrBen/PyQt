...
class MyWin(QMainWindow):
    ...
    def initUi(self):
        ...
        self.resize(100, 100)     # 界面的大小
        self.btn = QPushButton("截图", self)
        self.btn.setGeometry(20, 20, 60, 60)   # 给按钮指定位置
        self.btn.clicked.connect(self.click_btn)   # 给按钮绑定事件
...
class ScreenShotsWin(QMainWindow):
    ...
    def initUI(self):
        # self.showFullScreen()
        self.setWindowOpacity(0.4)   # 设置透明度
        self.btn_ok = QPushButton('保存', self)    # 设定按钮

        self.oksignal.connect(lambda: self.screenshots(self.start, self.end))    # 触发这个信号连接的槽函数
